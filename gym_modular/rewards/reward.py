...
class Reward(ABC, Generic[TaskType]):
    ...
    def reset(self) -> None:
        ...
        self.__max_reward = self._get_max_reward_unnormalized()
    ...
    def calculate_reward(self, done) -> float:
        ...
        if self.__reward_condition is None or self.__reward_condition():
            ...
            if self.__normalize:
                reward = (reward - self.__min_reward) / (self.__max_reward - self.__min_reward)
    ...
    def _get_min_reward_unnormalized(self) -> float:
        """
        Returns the lowest possible (unnormalized) reward per step.
    ...
    def _get_max_reward_unnormalized(self) -> float:
        """
        Returns the highest possible (unnormalized) reward per step.

        :return:            the highest possible (unnormalized) reward
        """
        pass
