...
class BaseNormalizer(torch.nn.Module):
    ...
    def observed_lower_bound_normalized(self) -> torch.Tensor:
        return self.normalize(self.observed_lower_bound_unnormalized)

    @torch.jit.export
    def observed_upper_bound_normalized(self) -> torch.Tensor:
        return self.normalize(self.observed_upper_bound_unnormalized)
...
class MeanVarNormalizer(BaseNormalizer):
    ...
    @torch.jit.export
    def normalize(self, unnormalized_values: torch.Tensor) -> torch.Tensor:
        return self._process(unnormalized_values, normalize=True, use_mean=True)

    @torch.jit.export
    def normalize_std_dev(self, unnormalized_std_devs: torch.Tensor) -> torch.Tensor:
        return self._process(unnormalized_std_devs, normalize=True, use_mean=False)

    @torch.jit.export
    def denormalize(self, normalized_values: torch.Tensor) -> torch.Tensor:
        return self._process(normalized_values, normalize=False, use_mean=True)

    @torch.jit.export
    def denormalize_std_dev(self, normalized_std_devs: torch.Tensor) -> torch.Tensor:
        return self._process(normalized_std_devs, normalize=False, use_mean=False)

    def _process(self, values: torch.Tensor, normalize: bool = True, use_mean: bool = True) -> torch.Tensor:
        ...
        if normalize:
            if use_mean:
                values = values - self._mean.reshape(shape)
            values = values / self.std_dev.reshape(shape)
        else:
            values = values * self.std_dev.reshape(shape)
            if use_mean:
                values = values + self._mean.reshape(shape)
